...
plt.tight_layout()
plt.show()
# Categorizing Age into Groups #Youth: Ages 0-25 #Middle-age: Ages 26-65 #Elderly: Ages 66 and above
...
plt.figure(figsize=(10, 6))
ax = sns.countplot(x='age_category', hue='short_flight_time', data=data.dropna(
    subset=['age_category', 'short_flight_time']), palette='viridis', order=labels)

plt.title('Distribution of Age Categories by Short Flight Time (with Percentages)')
...
total_counts_by_age = data.dropna(
    subset=['age_category', 'short_flight_time']).groupby('age_category').size()
...
for p in ax.patches:
    ...
    # Determine the exact age category from the order (Youth, Middle-age, Elderly)
    category_index = int(p.get_x() + 0.5)  # Roughly maps bar position to index
    current_age_category_label = labels[category_index]

    total_in_category = total_counts_by_age.get(current_age_category_label, 0)
